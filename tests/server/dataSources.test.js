...
import { loadDataSources, _resetForTests, getReadOnlyPool, closeAllReadOnlyPools } from '@server/dataSources.js';
...
afterEach(() => {
  ...
  delete process.env.DASHBOARD_READER_PASSWORD;
});
...
describe('loadDataSources', () => {
  ...
  it('merges entries from JSON file with dashboard_reader password from env', () => {
    fs.writeFileSync(tmpFile, JSON.stringify({
      sportly: {
        kind: 'app', scope: 'app', app_slug: 'sportly',
        db_ro: { host: 'localhost', port: 5432, database: 'sportly', user: 'dashboard_reader' }
      }
    }));
    process.env.DASHBOARD_READER_PASSWORD = 'shared-pw';
    const sources = loadDataSources(tmpFile);
    expect(sources.sportly.db_ro.password).toBe('shared-pw');
